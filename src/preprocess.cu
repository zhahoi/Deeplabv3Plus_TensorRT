...
// CUDA kernel to perform affine warp on the image （对图像做仿射变换，即对图像做放缩处理，进行通道转换BGR->RGB）
...
void cuda_preprocess(
    ...
    int img_size = src_width * src_height * 3;   // 计算图像大小（单位:字节）

    // Copy source image data to pinned host memory for faster transfer
    memcpy(img_buffer_host, src, img_size); // 将原图像复制到img_buffer_host

    // Asynchronously copy image data from host to device memory  
    CUDA_CHECK(cudaMemcpyAsync(  // 异步将图像拷贝到GPU设备内存 img_buffer_device
        img_buffer_device,
        img_buffer_host,
        img_size,
    ...
    // Define affine transformation matrices 构建仿射矩阵
    AffineMatrix s2d, d2s; // Source to destination and vice versa

    // Calculate the scaling factor to maintain aspect ratio 构造仿射变换矩阵（source->destination)
    float scale = std::min(
        dst_height / (float)src_height,
        dst_width / (float)src_width
    );

    // Initialize source-to-destination affine matrix (s2d) 等比缩放 + 中心对齐居中填充
    ...
    // Create OpenCV matrices for affine transformation 得到反向变换矩阵（destination -> source)
    ...
    // Launch the warp affine kernel 启动CUDA核函数（图像处理）
